...
exports.getShortestPath = async (req, res) => {
  ...
  try {
    ...
    while (reachableSlopes > 0) {
      ...
      if (currentSlopes) {  // Add this check
    ...
    while (currentSlopeId !== startSlopeId) {
      ...
      const currentSlope = prevSlopes.find(slope => slope.end._id.toString() === currentSlopeId);
      shortestPath.unshift(currentSlope);
      currentSlopeId = prevSlopeId;
    }

    // Return the shortest path in the same format as getAllPaths
    const allPaths = [];
...
//   difficultyLevels[currentSlope.difficultyLevel] = (difficultyLevels[currentSlope.difficultyLevel] || 0) + 1;
    ...
    const dfs = (currentId, path) => {
      ...
      if (currentId === endId) {
        ...
        if (currentSlopes) {
          currentSlopes.forEach(currentSlope => {
            const neighborId = currentSlope.end._id.toString();
            if (!path.includes(currentSlope)) { // Check if the path already includes the current slope
